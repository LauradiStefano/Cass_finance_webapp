...
def CGMY_ft(u, delta, N, dt, c, G, m, Y, r, S0, K):  # corretto
    ...
    output = np.exp(-r * N * dt) * (term - K * (N + 1) * CGMY_phi(0, u - 1j * delta, N, N, dt, c, G, m, Y, r, S0)) / (
            (N + 1) * (1j * u + delta))
...
def CGMY_option(S0, K, T, r, n, c, G, m, Y, Nfft, lmax, lmin, delta, tolerance):
    ...
    g0 = fr_fourier_transform(np.exp(-1j * u * l[0]) * CGMY_ft(u, delta, n, dt, c, G, m, Y, r, S0 / K, 1) * wght,
                              du * dl / (2 * math.pi))
