...
#        mu=0
#        omega=np.log(heston_charfn(N, Tt, v0, chi, lam, rho, v_hat, mu))
#        
#        
#        murn=r-q-np.real(omega);
    ...
    CharFn = heston_charfn(N, Tt, v0, chi, lam, rho, v_hat, mu)
...
def chiFO(c,d,N,a,b):
    ...
    chi = (1/(1+(rangeN*math.pi/(b-a))**2))*(np.cos(rangeN_d)*math.exp(d)-np.cos(rangeN_c)*math.exp(c)+(rangeN*math.pi/(b-a))*np.sin(rangeN_d)*math.exp(d)-(rangeN*math.pi/(b-a))*np.sin(rangeN_c)*math.exp(c));
