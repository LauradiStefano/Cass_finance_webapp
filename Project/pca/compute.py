...
# Permette di calcolare la matrice varianza covarianza importando i prezzi di
# chiusura da Yahoo
...
def variance_component_plot(evalues):
    ...
    fig = figure(x_range=tickers, plot_height=350, toolbar_location=None, title="Percentage Variance explained by each Component")
...
def cumulative_component_plot(evalues):
    ...
    fig = figure(x_range=tickers, plot_height=350, toolbar_location=None, title="Cumulative Percentage Variance Explained")
    fig.xaxis.axis_label = 'Ticker'
    fig.yaxis.axis_label = 'Percentage'
...
def loadings_plot_one(autovect):
    pca_one =autovect[:,0]


    tickers = [str(i) for i in range(autovect)]

    source = ColumnDataSource(data=dict(tickers=tickers, pca_one=pca_one))
...
def loadings_plot_two(autovect):
    pca_one =autovect[:,1]
