...
def z_charfn(u, dt, epsilon, k1, sigma):
    ...
    output = np.exp(1j*u*epsilon*(1-np.exp(-k1*dt))-(0.25/k1)*(1-np.exp(-2*k1*dt))*(sigma*u)**2)
...
def ONE_FACTOR_phi(g1, g2, n, N, dt, epsilon, k1, sigma, X0):
    ...
    output = np.exp(1j*(g1*np.exp(-n*dt*k1)+g2*np.mean(np.exp(-k1*dt*np.arange(1,N+1,1))))*X0)*m;
...
def ONE_FACTOR_FT(u, delta, N, dt, epsilon, k1, sigma, r, X0, K):
    ...
    output = np.exp(-r*N*dt)*(term-K*N*ONE_FACTOR_phi(0, u-1j*delta, N, N, dt, epsilon, k1, sigma, X0))/(N*(1j*u+delta));
...
def exp_gaussian_option(X0, K, T, r, n, epsilon, k1, sigma, Nfft, lmax, lmin, delta, tolerance):
    ...
    g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*ONE_FACTOR_FT(u, delta, n, dt, epsilon, k1, sigma, r, X0, K)*wght, du*dl/(2*math.pi))
