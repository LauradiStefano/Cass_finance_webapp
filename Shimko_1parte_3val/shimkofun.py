...
def ImpliedPDFReturns_FullRange(A0,A1,A2,SD,B,kret,kmin,kmax,x_fit_lgn): 
    pdf=ImpliedPDFPrices_FullRange(A0,A1,A2,SD,B,SD*math.exp(kret),kmin,kmax,x_fit_lgn)*SD*math.exp(kret)
    return pdf
...
def densF(A0,A1,A2,SD,B,k):
    eps=10^(-5)
    bs_deriv=lambda k:(bs(A0,A1,A2,SD,B,k+eps)-2*bs(A0,A1,A2,SD,B,k)+bs(A0,A1,A2,SD,B,k-eps))/(eps)**2;
    return bs_deriv(k)/B;
