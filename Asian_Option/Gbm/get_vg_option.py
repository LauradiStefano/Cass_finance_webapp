...
@author: Diego
"""
#from get_vg_ft import vg_ft
#from get_vg_fr_ft import fr_fourier_transform
import math
import numpy as np
import datetime
...
def vg_charfn(u, dt, sigma, theta, kappa, r):
    ...
    output = np.exp(dt*(1j*(r+np.log(1-theta*kappa-0.5*kappa*sigma**2)/kappa)*u))*(1/(1-1j*u*theta*kappa+0.5*kappa*(u*sigma)**2))**(dt/kappa)
...
def vg_ft(u, delta, N, dt, sigma, theta, kappa, r, S0, K):#corretto
    ...
    output = np.exp(-r*N*dt)*(term-K*(N+1)*vg_phi(0, u-1j*delta, N, N, dt, sigma, theta, kappa, r, S0))/((N+1)*(1j*u+delta));
...
def vg_option(S0, K, T, r, n, sigma, theta, kappa):
    ...
    g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*vg_ft(u, delta, n, dt, sigma, theta, kappa, r, S0/K, 1)*wght, du*dl/(2*math.pi))
