...
#from get_heston_ft import heston_ft
#from get_heston_fr_ft import fr_fourier_transform
import math
import numpy as np
import datetime
...
def heston_psi0(u1, u2, dt, r, a, b, eta, rho):
    ...
    output = 1j*u2*(r-rho*a*b/eta)*dt+(a*b/eta**2)*((a-w2)*dt-2*np.log((w1*np.exp(-w2*dt)-1)/(w1-1)))
...
def heston_psi1(u1, u2, dt, a, eta, rho):
    ...
    output = ((a-1j*u2*rho*eta-w2)-w1*np.exp(-w2*dt)*(a-1j*u2*rho*eta+w2))/((1-w1*np.exp(-w2*dt))*eta**2)
...
def heston_phi(g1, g2, n, N, dt, r, a, b, eta, rho, S0, v0):
    ...
    output = np.exp(1j*(g1+g2)*math.log(S0))*np.exp(heston_psi1(v, (g1+g2)*N/(N+1), dt, a, eta, rho)*v0)*np.exp(term)
...
def heston_ft(u, delta, N, dt, r, a, b, eta, rho, S0, v0, K):
    ...
    output = np.exp(-r*N*dt)*(term-K*(N+1)*heston_phi(0, u-1j*delta, N, N, dt, r, a, b, eta, rho, S0, v0))/((N+1)*(1j*u +delta))
...
def heston_option(S0, K, v0, T, r, n, a, b, eta, rho):
    ...
    g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*heston_ft(u, delta, n, dt, r, a, b, eta, rho, S0/K, v0, 1)*wght, du*dl/(2*math.pi))
