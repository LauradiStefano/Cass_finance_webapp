# -*- coding: utf-8 -*-
"""
Created on Mon May 20 15:03:38 2019
...
def CGMY_charfn(u, dt, c, G, m, Y, r):
    ...
    output = np.exp(dt*(1j*(r-c*math.gamma(-Y)*((m-1)**Y-m**Y+(G+1)**Y-G**Y))*u+c*math.gamma(-Y)*((m-1j*u)**Y-m**Y+(G+1j*u)**Y-G**Y)))
...
def CGMY_ft(u, delta, N, dt, c, G, m, Y, r, S0, K):#corretto
    ...
    for j in range(0,N+1):
        ...
        term = term + CGMY_phi(-1j, u-1j*delta, j, N, dt, c, G, m, Y, r, S0)
    
    output = np.exp(-r*N*dt)*(term-K*(N+1)*CGMY_phi(0, u-1j*delta, N, N, dt, c, G, m, Y, r, S0))/((N+1)*(1j*u+delta));
...
def CGMY_option(S0, K, T, r, n, c, G, m, Y, Nfft, lmax, lmin, delta, umax):
    ...
    g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*CGMY_ft(u, delta, n, dt, c, G, m, Y, r, S0/K, 1)*wght, du*dl/(2*math.pi))
