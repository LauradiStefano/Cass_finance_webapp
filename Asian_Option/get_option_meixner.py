...
def meixner_charfn(u, dt, a, b, d, r):
    ...
    output = np.exp(dt*(1j*(r-2*d*np.log(np.cos(b/2)/np.cosh(-1j*(a+b)/2)))*u))*(np.cos(b/2)/np.cosh((a*u-1j*b)/2))**(2*d*dt)
...
def meixner_ft(u, delta, N, dt, a, b, d, r, S0, K):#corretto
    ...
    output = np.exp(-r*N*dt)*(term-K*(N+1)*meixner_phi(0, u-1j*delta, N, N, dt, a, b, d, r, S0))/((N+1)*(1j*u+delta));
...
def meixner_option(S0, K, T, r, n, a, b, d, Nfft, lmax, lmin, delta, umax):
    ...
    while flag < 1:
        ...
        if abs(meixner_ft(umax, delta, n, dt, a, b, d, r, S0/K, 1))< 1E-5:
    ...
    g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*meixner_ft(u, delta, n, dt, a, b, d, r, S0/K, 1)*wght, du*dl/(2*math.pi))
