...
def mjd_charfn(u, dt, sigma, lam, mews, sigmas, r):
    ...
    output = np.exp(dt*(1j*(r-lam*(np.exp(mews+sigmas**2/2)-1)-sigma**2/2)*u+lam*(np.exp(1j*u*mews-(u*sigmas)**2/2)-1)-(u*sigma)**2/2));
...
def mjd_ft(u, delta, N, dt, sigma, lam, mews, sigmas, r, S0, K):#corretto
    ...
    output = np.exp(-r*N*dt)*(term-K*(N+1)*mjd_phi(0, u-1j*delta, N, N, dt, sigma, lam, mews, sigmas, r, S0))/((N+1)*(1j*u+delta));
...
def mjd_option(S0, K, T, r, n, sigma, lam, mews, sigmas, Nfft, lmax, lmin, delta, umax):
    ...
    g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*mjd_ft(u, delta, n, dt, sigma, lam, mews, sigmas, r, S0/K, 1)*wght, du*dl/(2*math.pi))
