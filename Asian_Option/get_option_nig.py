...
def nig_charfn(u, dt, a, b, d, r):
    ...
    output = np.exp(dt*(1j*(r+d*(np.sqrt(a**2-(b+1)**2)-np.sqrt(a**2-b**2)))*u-d*(np.sqrt(a**2-(b+1j*u)**2)-np.sqrt(a**2-b**2))))
...
def nig_ft(u, delta, N, dt, a, b, d, r, S0, K):#corretto
    ...
    output = np.exp(-r*N*dt)*(term-K*(N+1)*nig_phi(0, u-1j*delta, N, N, dt, a, b, d, r, S0))/((N+1)*(1j*u+delta));
...
def nig_option(S0, K, T, r, n, a, b, d, Nfft, lmax, lmin, delta, umax)):
    ...
    g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*nig_ft(u, delta, n, dt, a, b, d, r, S0/K, 1)*wght, du*dl/(2*math.pi))
