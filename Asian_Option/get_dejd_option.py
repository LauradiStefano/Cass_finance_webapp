...
def dejd_charfn(u, dt, sigma, lam, p, eta1, eta2, r):
    ...
    output = np.exp(dt*(1j*(r-lam*(p*eta1/(eta1-1) + (1-p)*eta2/(eta2+1)-1)-sigma**2/2)*u+lam*(p*eta1/(eta1-1j*u) + (1-p)*eta2/(eta2+1j*u)-1)-(u*sigma)**2/2));
...
def dejd_ft(u, delta, N, dt, sigma, lam, p, eta1, eta2, r, S0, K):#corretto
    ...
    output = np.exp(-r*N*dt)*(term-K*(N+1)*dejd_phi(0, u-1j*delta, N, N, dt, sigma, lam, p, eta1, eta2, r, S0))/((N+1)*(1j*u+delta));
...
def dejd_option(S0, K, T, r, n, sigma, lam, p, eta1, eta2, Nfft, lmax, lmin, delta, umax):
    ...
    g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*dejd_ft(u, delta, n, dt, sigma, lam, p, eta1, eta2, r, S0/K, 1)*wght, du*dl/(2*math.pi))
