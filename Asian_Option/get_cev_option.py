...
def factor2(m, gamma, dt, r, sig):
    ...
    s = sig*(2-gamma)
    output = m*((g-b)+np.exp(-g*dt)*(g+b))/(s**2*m*(1-np.exp(-g*dt))+(g+b)+np.exp(-g*dt)*(g-b))
...
def CEVFTGammaAboveTwo(u, delta, N, dt, r, sigma, gamma, X0, K):
    ...
    for j in range(0,N+1):
        term = term + X0**(1/(2-gamma))*np.exp(r*j*dt)*CEVSpotPhi(0, u-1j*delta, j, N, dt, r, sigma, gamma, X0)

    output = np.exp(-r*N*dt)*(-term+K*(N+1)*CEVPhi(0, u-1j*delta, N, N, dt, r, sigma, gamma, X0))/((N+1)*(1j*u+delta))
...
def cev_option(S0, K, F, T, r, n, gamma, sigma, Nfft, lmax, lmin, delta, umax):
    ...
    if gamma > 2:
        g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*CEVFTGammaAboveTwo(u, delta, n, dt, r, sigma, gamma, S0**(2-gamma), K)*wght, du*dl/(2*math.pi));
    else:
        g0 = fr_fourier_transform(np.exp(-1j*u*l[0])*CEVFT(u, delta, n, dt, r, sigma, gamma, S0**(2-gamma), K)*wght, du*dl/(2*math.pi));
